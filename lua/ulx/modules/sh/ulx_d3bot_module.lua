	...
	function ulx.killnests(caller)
		...
		if killedCount > 0 then
			if IsValid(caller) then
				caller:ChatPrint("Removed " .. killedCount .. " zombie nest(s).")
			else
				print("Removed " .. killedCount .. " zombie nest(s).")
			end
			-- Notify all players
			for _, pl in ipairs(player.GetAll()) do
				if not pl:IsBot() then
					pl:PrintMessage(HUD_PRINTTALK, "[ADMIN] " .. killedCount .. " zombie nest(s) destroyed.")
				end
			end
		else
			if IsValid(caller) then
				caller:ChatPrint("No zombie nests found.")
			else
				print("No zombie nests found.")
			end
		end
	end
	local killnests = ulx.command("ZS ULX Commands", "ulx killnests", ulx.killnests, "!killnests")
	killnests:defaultAccess(ULib.ACCESS_ADMIN)
	killnests:help("Removes all zombie nests from the map.")

	---------------------------------
	-- ZS ULX Commands
	---------------------------------

	-- Extend the current wave to 100 hours from now
	function ulx.waveduration100h(caller)
		-- Check if we're in a wave
		local currentWave = GAMEMODE:GetWave()
		if currentWave < 1 then
			local msg = "Cannot extend wave duration - no wave is active yet (still in wave 0 preparation)."
			if IsValid(caller) then
		...
		-- Set wave end to 100 hours from now (360000 seconds)
		local duration = 360000
		local newEndTime = CurTime() + duration

		-- Use the gamemode's SetWaveEnd function
		GAMEMODE:SetWaveEnd(newEndTime)

		local message = "Wave " .. currentWave .. " extended to 100 hours from now."
		if IsValid(caller) then
			caller:ChatPrint(message)
		else
			print(message)
		end
		...
		-- Notify all players
		for _, pl in ipairs(player.GetAll()) do
			if not pl:IsBot() and pl ~= caller then
				pl:PrintMessage(HUD_PRINTTALK, "[ADMIN] " .. message)
			end
		end
	end
	local waveduration100h = ulx.command("ZS ULX Commands", "ulx waveduration100h", ulx.waveduration100h, "!waveduration100h")
	waveduration100h:defaultAccess(ULib.ACCESS_ADMIN)
	waveduration100h:help("Extends the current wave to 100 hours from now.")
end
...
registerSuperadminCmd("SetMapParam", strParam, optionalStrParam, function(caller, name, serializedNumOrStrOrEmpty)
	D3bot.TryCatch(function()
		...
		if string.lower(name) == "botmod" then
			if D3bot and D3bot.EnsureForcedCadeBots then
				D3bot.EnsureForcedCadeBots()
			end
		end
	end, function(errorMsg)
		caller:ChatPrint("Error. Re-check your parameters.")
...
registerSuperadminCmd("KickBot", strRestParam, function(caller, botName)
	local targetBot = nil
	for _, pl in ipairs(player.GetAll()) do
		if pl:IsBot() and string.find(string.lower(pl:Nick()), string.lower(botName), 1, true) then
			targetBot = pl
			break
		end
	end
	...
	if targetBot then
		local botNick = targetBot:Nick()
		local botTeam = targetBot:Team() == TEAM_HUMAN and "Human" or (targetBot:Team() == TEAM_UNDEAD and "Zombie" or "Spectator")
		targetBot:Kick("Kicked by admin (!bot kickbot)")

		if IsValid(caller) then
			caller:ChatPrint("Kicked bot: " .. botNick .. " (" .. botTeam .. " team)")
		else
			print("Kicked bot: " .. botNick .. " (" .. botTeam .. " team)")
		end
	else
		if IsValid(caller) then
			caller:ChatPrint("No bot found matching name: " .. botName)
		else
			print("No bot found matching name: " .. botName)
		end
	end
end)

registerSuperadminCmd("KickZombies", function(caller)
	local kickedCount = 0
	for _, pl in ipairs(player.GetAll()) do
		if pl:IsBot() and pl:Team() == TEAM_UNDEAD then
			pl:Kick("Kicked by admin (!bot kickzombies)")
	...
	if kickedCount > 0 then
		if IsValid(caller) then
			caller:ChatPrint("Kicked " .. kickedCount .. " zombie bot(s).")
		else
			print("Kicked " .. kickedCount .. " zombie bot(s).")
		end
		-- Notify all players
		for _, pl in ipairs(player.GetAll()) do
			if not pl:IsBot() then
				pl:PrintMessage(HUD_PRINTTALK, "[ADMIN] " .. kickedCount .. " zombie bot(s) kicked from server.")
			end
		end
	else
		if IsValid(caller) then
			caller:ChatPrint("No zombie bots found to kick.")
		else
			print("No zombie bots found to kick.")
		end
	end
end)

registerSuperadminCmd("KickAllBots", function(caller)
	local kickedCount = 0
	for _, pl in ipairs(player.GetAll()) do
		if pl:IsBot() then
			pl:Kick("Kicked by admin (!bot kickallbots)")
			kickedCount = kickedCount + 1
		end
	end
	...
	if kickedCount > 0 then
		if IsValid(caller) then
			caller:ChatPrint("Kicked " .. kickedCount .. " bot(s) from all teams.")
		else
			print("Kicked " .. kickedCount .. " bot(s) from all teams.")
		end
		-- Notify all players
		for _, pl in ipairs(player.GetAll()) do
			if not pl:IsBot() then
				pl:PrintMessage(HUD_PRINTTALK, "[ADMIN] All bots kicked from server (" .. kickedCount .. " total).")
			end
		end
	else
		if IsValid(caller) then
			caller:ChatPrint("No bots found to kick.")
		else
...
registerSuperadminCmd("ProtectBot", strRestParam, function(caller, botName)
	local targetBot = nil
	for _, pl in ipairs(player.GetAll()) do
		if pl:IsBot() and string.find(string.lower(pl:Nick()), string.lower(botName), 1, true) then
			targetBot = pl
	...
	if targetBot then
		local botNick = targetBot:Nick()
		targetBot.D3bot_Protected = true

		if IsValid(caller) then
			caller:ChatPrint("Protected bot from auto-kick: " .. botNick)
		else
			print("Protected bot from auto-kick: " .. botNick)
		end
	else
		if IsValid(caller) then
			caller:ChatPrint("No bot found matching name: " .. botName)
...
registerSuperadminCmd("UnprotectBot", strRestParam, function(caller, botName)
	local targetBot = nil
	for _, pl in ipairs(player.GetAll()) do
		if pl:IsBot() and string.find(string.lower(pl:Nick()), string.lower(botName), 1, true) then
			targetBot = pl
	...
	if targetBot then
		local botNick = targetBot:Nick()
		targetBot.D3bot_Protected = false

		if IsValid(caller) then
			caller:ChatPrint("Unprotected bot (can be auto-kicked): " .. botNick)
		else
			print("Unprotected bot (can be auto-kicked): " .. botNick)
		end
	else
		if IsValid(caller) then
			caller:ChatPrint("No bot found matching name: " .. botName)
...
registerSuperadminCmd("ListProtected", function(caller)
	local protectedBots = {}
	for _, pl in ipairs(player.GetAll()) do
		if pl:IsBot() and pl.D3bot_Protected then
			local teamName = pl:Team() == TEAM_HUMAN and "Human" or (pl:Team() == TEAM_UNDEAD and "Zombie" or "Spec")
			table.insert(protectedBots, pl:Nick() .. " (" .. teamName .. ")")
		end
	end

	if #protectedBots > 0 then
		if IsValid(caller) then
			caller:ChatPrint("Protected bots (" .. #protectedBots .. "):")
			for _, info in ipairs(protectedBots) do
				caller:ChatPrint("  - " .. info)
			end
		else
			print("Protected bots (" .. #protectedBots .. "):")
			for _, info in ipairs(protectedBots) do
				print("  - " .. info)
			end
		end
		...
		if IsValid(caller) then
			caller:ChatPrint("No protected bots found.")
		else
			print("No protected bots found.")
		end
	end
end)
