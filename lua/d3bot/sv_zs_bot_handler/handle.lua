...
hook.Add("PlayerDeath", D3bot.BotHooksId.."PlayerDeath", function(pl, inflictor, attacker)
	if D3bot.IsEnabledCached and pl.D3bot_Mem then
		...
		-- СТРАТЕГІЧНИЙ ВИБІР FLESH CREEPER 
		-- Умови для спавну:
		-- 1. Бот у команді зомбі і помер природною смертю (не суїцид)
		-- 2. Ліміт Кріперів на карті не перевищено
		-- 3. Гнізд менше 2-ох АБО існуючі гнізда занадто далеко від людей (>900 юнітів)
		-- 4. Шанс 50% якщо умови вище виконані
		if pl:Team() == TEAM_UNDEAD then
			local zombieClassData = GAMEMODE.ZombieClasses[pl:GetZombieClass()]
			local currentClassName = zombieClassData and zombieClassData.Name or ""
			local isSuicide = (attacker == pl) or (not IsValid(attacker))
			
			if currentClassName ~= "Flesh Creeper" and not isSuicide then
				-- Збираємо позиції живих людей для перевірки дистанцій
				local humans = {}
				for _, p in ipairs(team.GetPlayers(TEAM_HUMAN)) do
					if IsValid(p) and p:Alive() and p:GetObserverMode() == OBS_MODE_NONE then
				...
				-- Рахуємо гнізда і перевіряємо їх ефективність
				if #humans > 0 then
					for _, nest in ipairs(ents.FindByClass("prop_creepernest")) do
						if IsValid(nest) and nest.GetNestBuilt and nest:GetNestBuilt() then
							...
							for _, human in ipairs(humans) do
								...
								if distSqr < closestHumanDistSqr then
									closestHumanDistSqr = distSqr
								end
							end
							
							-- Перевіряємо чи гніздо далі ніж 900 юнітів (900*900 = 810000)
							if closestHumanDistSqr >= 810000 then
								hasFarNests = true
							end
						end
					end
				else
					-- Якщо людей немає, просто рахуємо гнізда
					for _, nest in ipairs(ents.FindByClass("prop_creepernest")) do
						if IsValid(nest) and nest.GetNestBuilt and nest:GetNestBuilt() then
							builtNestsCount = builtNestsCount + 1
						end
					...
					for _, zombieClass in pairs(GAMEMODE.ZombieClasses) do
						if zombieClass.Name == "Flesh Creeper" then
							...
							if DEBUG_BEHAVIOR then
								print("[D3bot] Smart Spawn - Bot " .. pl:Nick() .. " became Flesh Creeper! (Nests: " .. builtNestsCount .. ", Far: " .. tostring(hasFarNests) .. ")")
							end
							break
						end
					end
