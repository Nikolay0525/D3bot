...
-- Змінні для КЕШУВАННЯ (щоб не лагало при масових вбивствах)
local CachedWave = -1
...
local function UpdateClassCache()
    ...
    -- Якщо хвиля та сама і кеш не пустий, нічого не робимо (економія ресурсів)
    ...
    local unlockedClasses = D3bot.ZS.GetUnlockedZombieClasses()
    ...
    -- Сортуємо списки один раз на всю хвилю
...
local function AreHumansUnreachable(bot)
    ...
    if IsValid(target) and target:IsPlayer() then
        local dist = bot:GetPos():DistToSqr(target:GetPos()) -- DistToSqr швидше за Distance!
        if dist > 2250000 then -- 1500 * 1500
			if DEBUG_BEHAVIOR then
				print("Bot choose to play as fast Z")
			end
            return true
        end
    end
...
local function GetSmartClassIndex(bot, prioritizeSpeed)
    UpdateClassCache() 
    ...
    if #cache == 0 then
        return GAMEMODE.ZombieClasses[GAMEMODE.DefaultZombieClass].Index
    end
...
function HANDLER.OnDeathFunction(bot)
    ...
    bot.DeathClass = GetSmartClassIndex(bot, humansUnreachable)
    HANDLER.RerollTarget(bot)
end
